...
public class Services extends AppCompatActivity {
    ...
    EditText et_amount;
    CalendarView cal;
    TextView date_textView, expense_list, view_Expenses, pound_textView, services_text, date_selected;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        add_btn.setOnClickListener(v -> {
            ...
            try {
                expensesModel = new ExpensesModel(-1, date_textView.getText().toString(), Double.parseDouble(et_amount.getText().toString()), services_text.getText().toString());
                Toast.makeText(Services.this, expensesModel.toString(), Toast.LENGTH_SHORT).show();
            } catch (Exception e) {
                expensesModel = new ExpensesModel(-1, "error", 0, "error");
                Toast.makeText(Services.this, "Error creating entry", Toast.LENGTH_SHORT).show();

            }

            DBHelper dataBaseHelper = new DBHelper(Services.this);
        ...
        view_Expenses.setOnClickListener(v -> {
            Intent expenseServices = new Intent(Services.this, Total.class);
            startActivity(expenseServices);
        });
