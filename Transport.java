...
public class Transport extends AppCompatActivity {
    ...
    EditText et_amount;
    CalendarView cal;
    TextView date_textView, expense_list, view_Expenses, pound_textView, transport_text, date_selected;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        add_btn.setOnClickListener(v -> {
            ...
            try {
                expensesModel = new ExpensesModel(-1, date_textView.getText().toString(), Double.parseDouble(et_amount.getText().toString()), transport_text.getText().toString());
                Toast.makeText(Transport.this, expensesModel.toString(), Toast.LENGTH_SHORT).show();
            } catch (Exception e) {
                expensesModel = new ExpensesModel(-1, "error", 0, "error");
                Toast.makeText(Transport.this, "Error creating entry", Toast.LENGTH_SHORT).show();

            }

            DBHelper dataBaseHelper = new DBHelper(Transport.this);
        ...
        view_Expenses.setOnClickListener(v -> {
            Intent expenseTotal = new Intent(Transport.this, Total.class);
            startActivity(expenseTotal);
        });
